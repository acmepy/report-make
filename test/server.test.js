...
test('montaje, archivos, revisiones y eliminación', async t => {
  ...
  const extra = { ...value, name: 'Variables', logo: 'data:image/png;base64,abc', header: { titulo: 'Factura' } };
  const variableResponse = await request('/api/templates/variables', 'PUT', extra);
  assert.equal(variableResponse.status, 201);
  const variableRecord = await variableResponse.json();
  assert.equal(variableRecord.logo, extra.logo);
  assert.deepEqual(variableRecord.header, extra.header);
  const changedResponse = await request('/api/templates/variables', 'PUT', { ...variableRecord, header: { titulo: 'Nuevo' } });
  const changedRecord = await changedResponse.json();
  assert.notEqual(changedRecord.revision, variableRecord.revision);
  assert.equal((await request('/api/templates/variables', 'PUT', variableRecord)).status, 409);
  const movedResponse = await request('/api/templates/variables', 'PUT', { ...changedRecord, name: 'Variables nuevas' });
  assert.equal(movedResponse.status, 200);
  const disk = JSON.parse(await readFile(path.join(directory, 'variables-nuevas.json'), 'utf8'));
  assert.equal(disk.logo, extra.logo);
  assert.equal(disk.header.titulo, 'Nuevo');
  assert.equal((await request('/api/templates/invalid-vars', 'PUT', { ...value, name: 'Invalid vars', numero: 1 })).status, 400);
  const moved = await movedResponse.json();
  delete moved.logo;
  const removed = await request('/api/templates/variables-nuevas', 'PUT', moved);
  assert.equal(removed.status, 200);
  assert.equal(Object.hasOwn(await removed.json(), 'logo'), false);
});
